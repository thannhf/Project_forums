<html>	
	<head>
		...
		<style>
			.Edit_block:hover, .Del_block:hover, .Update_block:hover {
	...
	<body>
		<?php
			...
			if (isset($_POST['btn_submit'])) {
				...
				if (empty($errors)) {
					$target = "\View\Admins/NewsForums/photo/" . basename($file_name);
				...
				echo "<div style='width:100%; height:300px;overflow-y:auto;overflow-x:hidden; border:1px solid orange; background-color:black;color:white;'>";
					...
					echo '<div>';
						echo '<form method="post" action="posts_add.php">';
							...
							echo '<textarea name="comment_content" style="resize:none; width:100%; height:100px;"></textarea><br>';
							echo '<button type="submit" name="btn_comment_submit" class="btnComment" style="cursor:pointer; padding: 7px; border:1px solid orange;">Bình luận</button>';
						echo '</form>';
					...
					// echo '<div style="width:auto; height:auto; display:flex; justify-content:space-arround; background-color: White;">';
					// 	echo '<div><a href="posts_edit.php?id='.$row['id'].'" class="Edit_block" style="width:auto; height:auto; padding: 7px; color: black; font-weight:bold; text-decoration:none; cursor:pointer;">Edit</a></div>|
					// 	<div><a href="posts_delete.php?id='.$row['id'].'" class="Del_block" style="width:auto; height:auto; padding: 7px; color: black; font-weight:bold; text-decoration:none; cursor:pointer;">Delete</a></div>|
					// 	<div><a href="posts_update.php?id='.$row['id'].'" class="Update_block" style="width:auto; height:auto; padding: 7px; color: black; font-weight:bold; text-decoration:none; cursor:pointer;">Update</a></div>';
					// echo '</div>';
				echo "</div><br><br><br>";
			}
			mysqli_close($conn);
